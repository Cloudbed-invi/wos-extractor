    ...
    def _decode_map_frame(self, frame: bytes):
        ...
        for i,st in enumerate(starts):
            ...
            try:
                ...
                anchor=None
            # --- Antigravity Patch: Extract chunk coordinates FIRST ---
            v_idx = 0
            ...
            while True:
                ...
                v_idx += 4
            # --- Antigravity Patch: Unlock Conservative Mode ---
            if not anchor:
                decoded_atlas = row.get('atlas_id')
                if decoded_atlas:
                    anchor = {'atlas_id': decoded_atlas, 'pseudo_display': row.get('pseudo_display'), 'alliance_tag': row.get('alliance_tag'), 'anchor_kind': 'unlocked-map-discovery'}
                else:
                    continue
